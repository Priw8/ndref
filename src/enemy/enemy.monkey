...
Class Enemy Extends MobileEntity Abstract
    ...
    Function CullEnemiesDownTo: Void(cullNum: Int)
        Local cullableEnemies := Enemy.GetNumCullableEnemies()
        'TODO: there are some logs generated by this function, but I did not bother with that because
        'the code decompiled by ghidra was complete garbage thanks to string operations
        Local limit := 5000
        While cullNum < cullableEnemies
            limit -= 1
            If limit <= 0
                Return
            End If
            ...
            If enemy <> Null And enemy.IsCullable()
                enemy.Cull()
                cullableEnemies -= 1
            End If
        End While
